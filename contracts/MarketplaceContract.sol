...
contract Marketplace is ERC721URIStorage {
    ...
    // prices for different Bubbl Pass durations
    
    // - monthlyStandardPassPrice: 0.0015 ETH ($3)
    // - annualStandardPassPrice: 0.01 ETH ($20)
    // - monthlyPremiumPassPrice: 0.003 ETH ($6)
    // - annualPremiumPassPrice: 0.02 ETH ($40)
    // - rentAddOnDailyPrice: 0.0005 ETH ($1)

    uint256 public constant monthlyStandardPassPrice = 0.0015 ether;
    uint256 public constant annualStandardPassPrice = 0.01 ether;

    uint256 public constant monthlyPremiumPassPrice = 0.003 ether;
    uint256 public constant annualPremiumPassPrice = 0.02 ether;

    uint256 public constant rentAddOnDailyPrice = 0.0005 ether;
    ...
    mapping(address => BubblPass) private _standardPasses;
    mapping(address => BubblPass) private _premiumPasses;
    ...
    constructor(address _contractOwner) ERC721("BUBBL MARKETPLACE", "BBL") {
    ...
    function listNFTForSale(uint256 tokenId, uint256 price) public {
        ...
        require(price > 0, "Price must be at least 1 wei."); // Assuming "sun" was a typo, changed to "wei" for clarity
    ...
    function purchaseStandardPass(BubblPassDuration duration) public payable {
        require(!hasActivePass(msg.sender), "User already has an active pass");

        uint256 price;
        uint256 validUntil;

        if (duration == BubblPassDuration.Monthly) {
            price = monthlyStandardPassPrice;
            validUntil = block.timestamp + 30 days;
        } else if (duration == BubblPassDuration.Annual) {
        ...
        BubblPass storage userPass = _standardPasses[msg.sender];
    ...
    function purchasePremiumPass(BubblPassDuration duration) public payable {
        require(!hasActivePass(msg.sender), "User already has an active pass");
        ...
        BubblPass storage userPass = _premiumPasses[msg.sender];
        userPass.holder = msg.sender;
        userPass.validUntil = validUntil;
    ...
    function getActivePass(
        address user
    ) internal view returns (BubblPass storage) {
    ...
    function calculateRentAddOnCost(
        ...
        BubblPass storage activePass = getActivePass(user);
